...
public class AuthenticationController {
	...
	@PostMapping("api/registration/resortowner")
	public ResponseEntity<?> registerResortOwner(@RequestBody RegistrationDTO dto) {
		try {
			ResortOwner user = (ResortOwner) userRegistrationMapper.convertToEntity(dto, ResortOwner.class);
			usersService.addNew(user);
			emailService.sendRegisterConfirmationMail(user);
			return ResponseEntity.ok(user);
		} catch (RegistrationException ex) {
