...
export default {
  ...
  methods: {
    createRequest() {
      ...
      this.$axios.get(`${config.apiStart}/api/appointments/${role}/${this.$route.params.idresort}/calendar/month?startDate=${startDateQuery}&endDate=${endDateQuery}`).then(resp=>{
          this.calendarDays=resp.data
        });

      this.$axios.get(`${config.apiStart}/api/${role}/${this.$route.params.idresort}/discount-offers?startDate=${startDateQuery}&endDate=${endDateQuery}`).then(resp=>{
          this.calendarDaysDiscountOffers=resp.data;
        });
        this.$axios.get(`${config.apiStart}/api/${role}/${this.$route.params.idresort}/standard-periods?startDate=${startDateQuery}&endDate=${endDateQuery}`).then(resp=>{
