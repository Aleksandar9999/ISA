...
export default {
  props: ["show", "idResortOwner", "selectResort", "userId"],
  data() {
    return {
      dialog: {
        show: false,
      ...
      appointmentLocal: {
        ...
        additionalServices: "",
        price: "",
        resortId: this.$route.params.idresort,
        userId: "",
        validateUser: true,
      },
    };
  ...
  mounted() {
     axios
      .get(
        `${config.apiStart}/api/users/resortowners/${this.id}/resorts`,
        config.requestHeader
      )
      .then((resp) => (this.resorts = resp.data));
    axios
      .get(config.apiStart + "/api/users/clients", config.requestHeader)
      .then((resp) => (this.clients = resp.data));

    if (this.selectResort)
      this.appointmentLocal.resortId = this.selectResort;
  ...
  methods: {
    ...
    save() {
      ...
      let api = "/api/appointments/resort";
       if (!this.userId) api += "/resortowner";
      axios
