<template>
  <div>
    <w-card class="info-card" no-border>
      <w-form no-keyup-validation no-blur-validation>
        <div class="text-right mt6" style="margin: 0px 0px 25px 0px">
          <w-button type="submit" :bg-color="subscribeButton.bgColor" color="white" v-if="showSubscribeButtonsFunc()" @click="subscribe" >{{subscribeButton.title}}</w-button>
          <w-button bg-color="blue" style="margin:0px 0px 0px 25px" color="white" v-if="showSubscribeButtonsFunc()" @click="showNewAppointmentDialog" >Make reservation</w-button>
        
        </div>
        <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p>Resort name:</p>
          </div>
          <div class="xs6 pa1">
            <w-input 
              class="mb3"
              style="color: black"
              v-model="resort_form.name"
              bg-color="white"
              border
              placeholder="Enter resort name"
              outline
            >
            </w-input>
          </div>
           </w-flex>

           <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p>Number of rooms:</p>
          </div>
          <div class="xs6 pa1">
            <w-input 
              class="mb3"
              style="color: black"
              v-model="resort_form.numOfRooms"
              bg-color="white"
              border
              placeholder="Enter number of rooms"
              outline
            >
            </w-input>
          </div>
        </w-flex>

           <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p>Number of beds per room:</p>
          </div>
          <div class="xs6 pa1">
            <w-input 
              class="mb3"
              style="color: black"
              v-model="resort_form.numOfBeds"
              bg-color="white"
              border
              placeholder="Enter number of beds"
              outline
            >
            </w-input>
          </div>
        </w-flex>

        <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p>Address</p>
          </div>
          <div class="xs6 pa1">
              <w-input
              
              class="mb3"
              style="color: black"
              v-model="resort_form.resortAddress.country"
              bg-color="white"
              border
              placeholder="Country"
              outline
            />
<w-input
              
              class="mb3"
              style="color: black"
              v-model="resort_form.resortAddress.city"
              bg-color="white"
              border
              placeholder="City"
              outline
            />

              <w-input
             
              class="mb3"
              style="color: black"
              v-model="resort_form.resortAddress.street"
              bg-color="white"
              border
              placeholder="Street"
              outline
            />
          </div>
        </w-flex>
        <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p style="margin-top: 3%">Short description</p>
          </div>
          <div class="xs6 pa1">
              <w-textarea
              class="mb3"
              style="color: black"
              v-model="resort_form.description"
              bg-color="white"
              border
              placeholder="Resort description"
              outline
            />
          </div>
        </w-flex>

        <w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p style="margin-top: 3%">Rules</p>
          </div>
          <div class="xs6 pa1">
            <w-textarea
              class="mb3"
              style="text-alignment:center"
              color: black
              v-model="resort_form.rules"
              bg-color="white"
              border
              placeholder="Rules"
              outline
            ></w-textarea>
          </div>
        </w-flex>

<w-flex wrap class="text-center">
          <div class="xs6 pa1">
            <p style="margin-top: 3%">Extras services</p>
          </div>
          <div class="xs6 pa1">
            <w-textarea
              class="mb3"
              style="text-alignment:center"
              color: black
              v-model="resort_form.extrasServices"
              bg-color="white"
              border
              placeholder="Extras Services"
              outline
            ></w-textarea>
          </div>
        </w-flex>


        <div v-if="showAdminButton" class="text-right mt6">
          <w-button type="submit" bg-color="green" color="white" @click="save">Save</w-button>
        </div>
      </w-form>
    </w-card>
  </div>
</template>

<script>
import config from '../../../../configuration/config';
export default {
  props: ["resort_info","resortOwnerId","showNewAppointmentDialog","showAdminButton"],
  data() {
    return {
      resort_form: {},
      subscribeButton:{
        title:'SUBSCRIBE',
        bgColor:'red'
      }
    };
  },
  methods: {
    isSubscribed(){
      this.$axios.get(`${config.apiStart}/api/resorts/${this.resort_info.id}/customers/me`).then(resp=>{
        this.subscribeButton.title="UNSUBSCRIBE"
        this.subscribeButton.bgColor=''
        console.log(resp)
      }).catch(()=>{
        this.subscribeButton.title="SUBSCRIBE"
        this.subscribeButton.bgColor='red'
      })
    },
    showSubscribeButtonsFunc(){
      if (localStorage.roles)
        return localStorage.roles.trim() === "ROLE_USER";
    },
    subscribe(){
      if(this.subscribeButton.title=='SUBSCRIBE')
        this.$axios.post(`${config.apiStart}/api/resorts/${this.resort_info.id}/customers`,this.resort_form).then(()=>this.isSubscribed()
        );
      else this.$axios.delete(`${config.apiStart}/api/resorts/${this.resort_info.id}/customers`,this.resort_form).then(()=>this.isSubscribed()); 
      
    },
    save() {
      console.log(this.resort_form);
      this.$axios.put(config.apiStart+"/api/resorts/"+this.resort_form.id,this.resort_form,config.requestHeader)
      .then(resp=>{this.resort_form=resp.data; alert("Updated");})
      .catch((err)=>{
        alert(err.response.data.message)
       })
    },
    nameChanged($event) {
      this.resort_form.name = $event.data;
      this.$emit("input", this.resort);
    },
  },
  watch: {
    resort_info: {
      immediate: true,
      handler(resortFromProps) {
        if (resortFromProps) {
          this.resort_form = {
            ...this.resort_form,
            ...resortFromProps,
          };
          this.isSubscribed()
        }
      },
    },
  },
};
</script>
<style>
p {
  text-align: start;
  color: white;
  font-size: 15px;
}
.flex-container {
  display: flex;
}

.flex-child {
  flex: 1;
}
div.row {
  margin-top: 15px;
  margin-left: 25px;
}
.flex-child:first-child {
  margin-right: 10%;
}
input.text-box-forma {
  margin: 0%;
}
label {
  left: 0;
  right: 0;
  top: 5%;
}

.info-card {
  margin: 0% 10%;
  padding: 3%;
}
textarea{
  max-width: inherit;
  color: black;
  text-align: center;
}
</style>
