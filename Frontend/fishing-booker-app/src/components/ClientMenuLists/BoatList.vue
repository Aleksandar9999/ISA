        ...
        <table class="r-table" cellspacing="0" cellpadding="0" border="0">
          ...
          <tbody class="tbl-content" v-for="item in dataList" :key="item">
            <tr>
              ...
              <td>{{ item.navigationEquipment }}</td>
              <td>{{ item.rate }}</td>
              <td><w-button v-if="showDeleteButton" @click="deleteBoat(item)">Delete</w-button></td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    deleteBoat(item) {
      console.log(item);
      axios
        .delete(`${config.apiStart}/api/boats/${item.id}`, config.requestHeader)
        .then(() => {
          alert("Deleted");
          this.fetchData()
        });
    ...
    sort() {
      ...
      if (criteria === "name_asc") {
        ...
        this.dataList.sort((a, b) =>
          a.boatAddress.city > b.boatAddress.city
            ? 1
            : b.boatAddress.city > a.boatAddress.city
            ? -1
            : 0
        );
