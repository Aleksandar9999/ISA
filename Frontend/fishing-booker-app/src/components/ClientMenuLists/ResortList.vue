        ...
        <table class="r-table" cellspacing="0" cellpadding="0" border="0">
          ...
          <tbody class="tbl-content" v-for="item in dataList" :key="item">
            <tr>
              ...
              <td>
                {{
                  item.resortAddress.street +
                  ", " +
                  item.resortAddress.city +
                  ", " +
                  item.resortAddress.country
                }}
              </td>
              <td>{{ item.numOfRooms }}</td>
              <td>{{ item.rate }}</td>
              <td><w-button @click="deleteResort(item)">DELETE</w-button></td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    deleteResort(resort) {
      axios
        .delete(
          `${config.apiStart}/api/resorts/${resort.id}`,
    ...
    sort() {
      ...
      } else if (criteria === "location_asc") {
        this.dataList.sort((a, b) =>
          a.resortAddress.city > b.resortAddress.city
            ? 1
            : b.resortAddress.city > a.resortAddress.city
            ? -1
            : 0
        );
